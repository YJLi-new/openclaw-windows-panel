        ...
        private sealed class MainForm : Form
        {
            ...
            private string BuildWslNativeStartCommand()
            {
                ...
                return string.Join(
                    ...
                    "PID_FILE=/tmp/openclaw-native-gateway.pid",
                    "kill_listener_pids() {",
                    ...
                    "        return 0",
                    "      fi",
                    "    elif ! command -v curl >/dev/null 2>&1 && ss -ltn 2>/dev/null | grep -F \":" + port + "\" >/dev/null 2>&1; then",
                    "      stable_hits=$((stable_hits + 1))",
                    "      if [ \"$stable_hits\" -ge 2 ]; then",
                    "        return 0",
                    "      fi",
                    ...
                    "      [ -n \"$health_code\" ] || health_code=\"000\"",
                    "    fi",
                    "    if ! ss -ltn 2>/dev/null | grep -F \":" + port + "\" >/dev/null 2>&1 && [ \"$health_code\" != \"200\" ]; then",
                    "      stable_hits=$((stable_hits + 1))",
                    "      if [ \"$stable_hits\" -ge 2 ]; then",
                    "        return 0",
                    "      fi",
                    ...
                    "kill_pid_file_process",
                    "kill_listener_pids",
                    "wait_for_gateway_down >/dev/null 2>&1 || true",
                    ...
                    "  exit 0",
                    "fi",
                    "tail -n 40 /tmp/openclaw-native-gateway.log 2>/dev/null || true",
                    "rm -f \"$PID_FILE\"",
                    "echo \"ok=0\"",
                    "exit 1");
            }
            ...
            private string BuildWslNativeStopCommand()
            {
                ...
                return string.Join(
                    ...
                    "  rm -f \"$PID_FILE\"",
                    "fi",
                    "\"$OPENCLAW_BIN\" gateway stop >/tmp/openclaw-native-stop.log 2>&1 || true",
