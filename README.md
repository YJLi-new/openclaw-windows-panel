<div align="center">
  <img src="assets/banner.jpg" alt="OpenClaw Control Panel Banner" width="100%" />
...
这次我额外补了一套“**Admin WSL bridge**”包装层，适合下面这种实际场景：

- OpenClaw 已经迁到 `WSL Ubuntu-24.04`
- Windows 侧仍然想保留一个可点的 `.exe` GUI
- 不希望每次都手敲 `wsl.exe` 或 PowerShell 长命令
...
- `scripts/package-admin-wsl-control-panel.sh`

推荐交付方式不是直接动旧 `.exe` 内部，而是：

1. 继续使用现成的 `openclaw-control-panel.exe`
2. 把 `win_native` 的 command/install command 指到新的 bridge 脚本
3. 由新的 bridge 脚本去调 WSL 里的 OpenClaw
...
- `dist/openclaw-control-panel-admin-wsl/`
- `dist/openclaw-control-panel-admin-wsl.zip`
...
> 💡 **提示**：OpenClaw 控制台 UI 通常运行在 `http://127.0.0.1:18790/`（具体取决于网关配置）。<br>
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\openclaw-find-runtime-paths.ps1
```
...
当前仓库同时包含已编译的 `openclaw-control-panel.exe` 和源码目录 `src/OpenClawControlPanel/`。如果您只是使用成品，可以直接运行 exe；如果您要继续改 UI 或命令逻辑，可以直接改源码并重新编译。
...
- 仍不确定时，再跑 `scripts/openclaw-find-runtime-paths.ps1`
...
```json
{
  ...
  "dashboard": {
    "gateway_root_url": "http://127.0.0.1:18790/"
  },
  "network": {
    "proxy_mode": "off"
  }
...
### 隐私保护设计
本仓库被设计为绝对干净的公共发布版：
- ✅ **配置文件留白**：仓库内保留安全的空白模板 `config/openclaw-control-panel-settings.template.json`，打包后仍会展平为发布目录根级文件。
...
- 🔎 **`scripts/openclaw-find-runtime-paths.ps1`** (定位真实运行时目录 / settings)
...
- ❌ **明明改了 JSON，但界面还是旧目录 / 旧地址**
  - 先运行 `scripts/openclaw-find-runtime-paths.ps1`。
